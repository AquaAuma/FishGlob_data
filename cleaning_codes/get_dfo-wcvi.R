...
WCVI_effort <- read_csv(
  ...
  dplyr::select(Trip.identifier, Set.number,Survey.Year,Set.date, Trip.start.date,
...
regions <- levels(as.factor(clean_wcvi$survey))

#run flag_spp function in a loop
for (r in regions) {
  flag_spp(clean_wcvi, r)
}

######### Apply trimming per survey_unit method 1
#apply trimming for hex size 7
dat_new_method1_hex7 <- apply_trimming_per_survey_unit_method1(clean_wcvi, 7)

#apply trimming for hex size 8
dat_new_method1_hex8 <- apply_trimming_per_survey_unit_method1(clean_wcvi, 8)

######### Apply trimming per survey_unit method 2
dat_new_method2 <- apply_trimming_per_survey_unit_method2(clean_wcvi)


#-------------------------------------------------------------------------------------------#
#### ADD STRANDARDIZATION FLAGS ####
#-------------------------------------------------------------------------------------------#
surveys <- sort(unique(clean_wcvi$survey))
survey_units <- sort(unique(clean_wcvi$survey_unit))
survey_std <- clean_wcvi %>% 
...
# Just run this routine should be good for all
write_clean_data(data = survey_std, survey = "WCVI_std",
