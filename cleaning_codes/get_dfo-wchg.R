...
regions <- levels(as.factor(clean_wchg$survey))

#run flag_spp function in a loop
for (r in regions) {
  flag_spp(clean_wchg, r)
}

######### Apply trimming per survey_unit method 1
#apply trimming for hex size 7
dat_new_method1_hex7 <- apply_trimming_per_survey_unit_method1(clean_wchg, 7)

#apply trimming for hex size 8
dat_new_method1_hex8 <- apply_trimming_per_survey_unit_method1(clean_wchg, 8)

######### Apply trimming per survey_unit method 2
dat_new_method2 <- apply_trimming_per_survey_unit_method2(clean_wchg)


#-------------------------------------------------------------------------------------------#
#### ADD STRANDARDIZATION FLAGS ####
#-------------------------------------------------------------------------------------------#
surveys <- sort(unique(clean_wchg$survey))
survey_units <- sort(unique(clean_wchg$survey_unit))
survey_std <- clean_wchg %>% 
...
survey_std |>
  ...
  summarize(hex7_0 = sum(!is.na(flag_trimming_hex7_0)),
            ...
            trim_2 = sum(!is.na(flag_trimming_2))) # number of hauls doesn't match DFO-WCGH_stats_hauls.csv but does match DFO-WCGH_hauls_removed.csv. Odd.
