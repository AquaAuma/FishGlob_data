...
write_clean_data(data = clean_hs, survey = "HS", overwrite = T, rdata=TRUE, csv = T)
...
survey_std |>
  ...
  summarize(hex7_0 = sum(!is.na(flag_trimming_hex7_0)),
            ...
            trim_2 = sum(!is.na(flag_trimming_2))) # number of hauls doesn't match DFO-HS_stats_hauls.csv but does match DFO-HS_hauls_removed.csv. Odd.
